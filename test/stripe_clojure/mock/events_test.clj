...
(deftest ^:integration retrieve-event-test
  (testing "Retrieve event"
    (let [response (events/retrieve-event stripe-client "evt_mock")]
      (is (map? response))
...
(deftest ^:integration list-events-test
  (testing "List events"
    (let [response (events/list-events {:limit 2})]
      (is (map? response))
