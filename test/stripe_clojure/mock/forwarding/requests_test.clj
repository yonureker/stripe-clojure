...
(deftest ^:integration create-request-test
  (testing "Create a financial connections request using stripe‑mock with required parameters"
    (let [params {:payment_method "pm_card_visa"
          ...
          response (requests/create-request stripe-client params)]
      (is (map? response))
      (is (= "forwarding.request" (:object response)))
      (is (string? (:id response))))))

(deftest ^:integration retrieve-request-test
  (testing "Retrieve a financial connections request using a dummy request id"
    (let [dummy-id "req_mock"
          response (requests/retrieve-request stripe-client dummy-id)]
      (is (map? response))
      (is (= "forwarding.request" (:object response)))
      (is (string? (:id response))))))

(deftest ^:integration list-requests-test
  (testing "List financial connections requests using stripe‑mock"
    (let [response (requests/list-requests stripe-client {})]
      (is (map? response))
