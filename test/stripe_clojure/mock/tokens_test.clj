...
(deftest ^:integration create-card-token-test
  (testing "Create card token"
    (let [params {:card {:number "4242424242424242"
                         :exp_month 12
                         :exp_year 2025
                         :cvc "123"}}
          response (tokens/create-card-token stripe-client params)]
      (is (map? response))
...
(deftest ^:integration create-bank-account-token-test
  (testing "Create bank account token"
    (let [params {:bank_account {:country "US"
          ...
          response (tokens/create-bank-account-token stripe-client params)]
      (is (map? response))
      (is (= "token" (:object response)))
      (is (string? (:id response))))))

(deftest ^:integration create-pii-token-test
  (testing "Create pii token"
    (let [params {:pii {:id_number "000000000"}}
          response (tokens/create-pii-token stripe-client params)]
      (is (map? response))
      (is (= "token" (:object response)))
      (is (string? (:id response))))))

(deftest ^:integration create-account-token-test
  (testing "Create account token"
    (let [params {:account {:business_type "individual"
                            :individual {:first_name "John"
                                         :last_name "Doe"}}}
          response (tokens/create-account-token stripe-client params)]
      (is (map? response))
      (is (= "token" (:object response)))
      (is (string? (:id response))))))

(deftest ^:integration create-person-token-test
  (testing "Create person token"
    (let [params {:person {:first_name "John" :last_name "Doe" :relationship {:owner true}}}
          response (tokens/create-person-token stripe-client params)]
      (is (map? response))
      (is (= "token" (:object response)))
      (is (= "card" (:type response)))
      (is (string? (:id response))))))

(deftest ^:integration create-cvc-update-token-test
  (testing "Create cvc update token"
    (let [params {:cvc_update {:cvc "123"}}
          response (tokens/create-cvc-update-token stripe-client params)]
      (is (map? response))
      (is (= "token" (:object response)))
      (is (string? (:id response))))))
...
(deftest ^:integration retrieve-token-test
  (testing "Retrieve token"
    (let [response (tokens/retrieve-token stripe-client "tok_mock")]
      (is (map? response))
      (is (= "token" (:object response)))
      (is (string? (:id response)))))) 