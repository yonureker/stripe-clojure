...
(deftest create-invoice-test
  (testing "create-invoice returns a valid invoice"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})]
...
(deftest create-preview-invoice-test
  (testing "create-preview-invoice returns a valid invoice preview"
    (let [dummy-customer "cus_mock_123"
          preview (invoices/create-preview-invoice stripe-client {:customer dummy-customer})]
...
(deftest update-invoice-test
  (testing "update-invoice returns an updated invoice"
    (let [dummy-customer "cus_mock_123"
          ...
          updated (invoices/update-invoice stripe-client invoice-id {:metadata {"test" "value"}})]
...
(deftest retrieve-invoice-test
  (testing "retrieve-invoice returns the correct invoice"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          retrieved (invoices/retrieve-invoice stripe-client invoice-id)]
...
(deftest retrieve-upcoming-test
  (testing "retrieve-upcoming returns a valid upcoming invoice"
    (let [dummy-customer "cus_mock_123"
          upcoming (invoices/retrieve-upcoming stripe-client {:customer dummy-customer})]
      (is (map? upcoming))
      (is (= "invoice" (:object upcoming))))))

(deftest list-invoices-test
  (testing "list-invoices returns a list response"
    (let [resp (invoices/list-invoices stripe-client {:limit 1})]
      (is (map? resp))
      (is (vector? (:data resp))))))
...
(deftest delete-invoice-test
  (testing "delete-invoice returns the deleted invoice"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          deleted (invoices/delete-invoice stripe-client invoice-id)]
...
(deftest finalize-invoice-test
  (testing "finalize-invoice finalizes a draft invoice"
    (let [dummy-customer "cus_mock_123"
          ...
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          finalized (invoices/finalize-invoice stripe-client invoice-id)]
...
(deftest mark-invoice-uncollectible-test
  (testing "mark-invoice-uncollectible marks an invoice as uncollectible"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          uncollectible (invoices/mark-invoice-uncollectible stripe-client invoice-id)]
...
(deftest pay-invoice-test
  (testing "pay-invoice processes invoice payment"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          paid (invoices/pay-invoice stripe-client invoice-id {:payment_method "pm_card_visa"})]
      (is (= "invoice" (:object paid))))))

(deftest search-invoices-test
  (testing "search-invoices returns matching invoices"
    (let [resp (invoices/search-invoices stripe-client {:query "status:'draft'"})]
      (is (map? resp))
      (is (vector? (:data resp))))))
...
(deftest send-invoice-test
  (testing "send-invoice sends an invoice to a customer"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          sent (invoices/send-invoice stripe-client invoice-id)]
...
(deftest void-invoice-test
  (testing "void-invoice voids an invoice"
    (let [dummy-customer "cus_mock_123"
          invoice (invoices/create-invoice stripe-client {:customer dummy-customer})
          invoice-id (:id invoice)
          voided (invoices/void-invoice stripe-client invoice-id)]
...
(deftest list-line-items-test
  (testing "list-line-items returns a list of line items"
    (let [dummy-invoice-id "inv_mock_123"
          resp (invoices/list-line-items stripe-client dummy-invoice-id {:limit 1})]
      (is (map? resp))
      (is (vector? (:data resp))))))

(deftest list-upcoming-line-items-test
  (testing "list-upcoming-line-items returns upcoming line items"
    (let [resp (invoices/list-upcoming-line-items stripe-client {:customer "cus_mock_123" :limit 1})]
      (is (map? resp))
      (is (vector? (:data resp))))))
...
(deftest update-line-item-test
  (testing "update-line-item updates an invoice line item"
    (let [dummy-invoice-id "inv_mock_123"
          ...
          updated (invoices/update-line-item stripe-client dummy-invoice-id dummy-line-item-id {:description "updated description"})]
      (is (map? updated))
...
(deftest add-line-items-test
  (testing "add-line-items adds line items to an invoice"
    (let [dummy-invoice-id "inv_mock_123"
          added (invoices/add-line-items stripe-client dummy-invoice-id {:lines [{:amount 500
                                                                    :currency "usd"
...
(deftest remove-line-items-test
  (testing "remove-line-items removes line items from an invoice"
    (let [dummy-invoice-id "inv_mock_123"
          removed (invoices/remove-line-items stripe-client dummy-invoice-id {:lines [{:id "il_mock_123"
                                                                         :behavior "delete"}]})]
      (is (map? removed))
...
(deftest update-line-items-test
  (testing "update-line-items updates line items on an invoice"
    (let [dummy-invoice-id "inv_mock_123"
          updated (invoices/update-line-items stripe-client dummy-invoice-id {:lines [{:id "il_mock_123" :description "updated line"}]})]
      (is (map? updated))
      (is (= "invoice" (:object updated))))))
