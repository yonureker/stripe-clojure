...
(deftest ^:integration list-credit-balance-transactions-test
  (testing "List credit balance transactions using a dummy customer id"
    (let [response (cbt/list-credit-balance-transactions stripe-client {:customer dummy-customer-id})]
      (is (map? response))
      (is (= "list" (:object response)))
      (is (vector? (:data response))))))

(deftest ^:integration retrieve-credit-balance-transaction-test
  (testing "Retrieve a credit balance transaction if available"
    (let [transactions (cbt/list-credit-balance-transactions stripe-client {:customer dummy-customer-id})
          transaction-id (some-> (:data transactions) first :id)]
      (if transaction-id
        (let [retrieved (cbt/retrieve-credit-balance-transaction stripe-client transaction-id)]
