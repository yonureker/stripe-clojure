...
(deftest create-value-list-item-test
  (testing "Create a radar value list item"
    (let [params {:value "test_value" :value_list "vl_mock"}
          response (vli/create-value-list-item stripe-mock-client params)]
      (is (map? response))
...
(deftest retrieve-value-list-item-test
  (testing "Retrieve a radar value list item"
    (let [response (vli/retrieve-value-list-item stripe-mock-client "vli_mock")]
      (is (map? response))
      (is (= "radar.value_list_item" (:object response)))
      (is (string? (:id response)))
...
(deftest list-value-list-items-test
  (testing "List radar value list items"
    (let [response (vli/list-value-list-items stripe-mock-client {:limit 3 :value_list "vl_mock"})]
      (is (map? response))
...
(deftest delete-value-list-item-test
  (testing "Delete a radar value list item"
    (let [response (vli/delete-value-list-item stripe-mock-client "vli_mock")]
      (is (map? response))
      (is (= "radar.value_list_item" (:object response)))
      (is (string? (:id response)))
