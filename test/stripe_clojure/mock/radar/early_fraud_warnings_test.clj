...
(deftest ^:integration retrieve-early-fraud-warning-test
  (testing "Retrieve early fraud warning"
    (let [response (efw/retrieve-early-fraud-warning stripe-client "efw_mock")]
      (is (map? response))
...
(deftest ^:integration list-early-fraud-warnings-test
  (testing "List early fraud warnings"
    (let [response (efw/list-early-fraud-warnings stripe-client {})]
      (is (map? response))
